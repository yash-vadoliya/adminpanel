...
function Home() {
  ...
  const uniqueCities = [...new Map(city.map(c => [c.city_name, c])).values()];

  const filteredFromCities = uniqueCities.filter(c =>
    c.city_name.toLowerCase().includes(from.toLowerCase())
  );

  const filteredToCities = uniqueCities.filter(c =>
    c.city_name.toLowerCase().includes(to.toLowerCase())
  );
  ...
  const handlePaymentSuccess = async (paymentResponse) => {
    ...
    if (ok) {
      ...
      const startCity = city.find(
        c => Number(c.city_id) === Number(selectedTrip.start_city_id)
      );

      const endCity = city.find(
        c => Number(c.city_id) === Number(selectedTrip.end_city_id)
      );
  ...
  const generatePDF = ({
    ...
    // img.src = `${selectedBus.travel_logo}`;
            ...
            <div className="row g-3 align-items-center">
              <div className="col-md">
                ...
                <div className="position-relative">
                  <input
                    className="form-control rounded-pill" placeholder="From City" value={from}
                    onChange={(e) => { setFrom(e.target.value); setShowFromList(true); }} onFocus={() => setShowFromList(true)} />
                  ...
                  {showFromList && filteredFromCities.length > 0 && (
                    <div className="list-group position-absolute w-100 shadow"
                      ...
                      {filteredFromCities.map(c => (
                        <button key={c.city_id} type="button" className="list-group-item list-group-item-action"
                          onClick={() => { setFrom(c.city_name); setShowFromList(false); }}>
                          {c.city_name}
                        </button>
                      ))}
                    </div>
              ...
              <div className="col-md">
                ...
                <div className="position-relative">
                  <input
                    className="form-control rounded-pill" placeholder="To City" value={to}
                    onChange={(e) => { setTo(e.target.value); setShowToList(true) }} onFocus={() => setShowToList(true)} />
                  ...
                  {showToList && filteredToCities.length > 0 && (
                    <div className="list-group position-absolute w-100 shadow"
                      ...
                      {filteredToCities.map(c => (
                        <button key={c.city_id} type="button" className="list-group-item list-group-item-action"
                          onClick={() => { setTo(c.city_name); setShowToList(false); }}>
                          {c.city_name}
                        </button>
                      ))}
                    </div>
            ...
            <div className="seat-panel shadow-lg">
              ...
              <div className="p-4">
                <div className="card p-3 mb-3">
                  ...
                  <RazorpayButton amount={totalAmount} onSuccess={handlePaymentSuccess} />
                  {paymentMessage && (
                    <div className="alert alert-success text-center mt-3" nav>
                      {paymentMessage}
                    </div>
                  )}
